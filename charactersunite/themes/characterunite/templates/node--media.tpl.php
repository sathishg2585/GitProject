		...
		for($i=0;$i<$sec_count;$i++) {
			...
			if (isset($content['field_media_sections'][$i]['entity']['field_collection_item'][$sec_value]['field_media_section_body']['#items']) && count($content['field_media_sections'][$i]['entity']['field_collection_item'][$sec_value]['field_media_section_body']['#items']) > 0) {
				...
				if ($section_place == 'Left' || $section_place == '') {
					...
					if ( isset($content['field_media_sections'][$i]['entity']['field_collection_item'][$sec_value]['field_media_section_body_more']['#items'])  && $content['field_media_sections'][$i]['entity']['field_collection_item'][$sec_value]['field_media_section_body_more']['#items'][0]['value'] == 1) {
						$section_body = '<div class="infomorebody">'. $section_body .' </div><span style="display: none;" class="infomorep"></span>';
					}
					if ($section_background_color == 'Grey') {
						$right_pane .= '<div style="">'.$section_body . '<aside class="share">'.render($region['spread_the_word']) . '</aside></div>';
			...
			if (isset($content['field_media_sections'][$i]['entity']['field_collection_item'][$sec_value]['field_media_section_file']['#items']) && count($content['field_media_sections'][$i]['entity']['field_collection_item'][$sec_value]['field_media_section_file']['#items']) > 0) {
				...
				if ($section_place == 'Left' || $section_place == '') {
					...
					$right_pane .= '<div style="width:100%;padding:5px;"><p>
									<a href="'.$section_file.'" target="_blank" class="cta uppercase">DOWNLOAD &raquo;</a>
								</p></div>';
				}							
			}
			if (isset($content['field_media_sections'][$i]['entity']['field_collection_item'][$sec_value]['field_media_image_section']['#items']) && count($content['field_media_sections'][$i]['entity']['field_collection_item'][$sec_value]['field_media_image_section']['#items']) > 0) {
				...
				for($j = 0; $j < $img_count; $j++)
				{
					...
					if ( isset($content['field_media_sections'][$i]['entity']['field_collection_item'][$sec_value]['field_media_image_section'][$j]['entity']['field_collection_item'][$img_value]['field_media_image_title']['#items']))
					...
					if ( isset($content['field_media_sections'][$i]['entity']['field_collection_item'][$sec_value]['field_media_image_section'][$j]['entity']['field_collection_item'][$img_value]['field_media_image_desc']['#items']))
					...
					if ($section_place == 'Left' || $section_place == '') {
						$left_pane .= '<div class="relatedVideos clearfix ">
										<a href="'.$section_image_href.'" class="relatedVideosImage ">
											<img alt="'.$section_image_title.'" src="'.$media_file_path.$section_image_file.'">
										</a>
						...
						$right_pane .= '
									<section class="copy">
										<a id="'.$section_image_title.'" href="'.$section_image_href.'">
											<img class="featured" src="'.$media_file_path.$section_image_file.'" alt="'.$section_image_title.'" border="0" />
										</a>
									</section>					
						';
					}
			...
			if (isset($content['field_media_sections'][$i]['entity']['field_collection_item'][$sec_value]['field_media_video_section']['#items']) && count($content['field_media_sections'][$i]['entity']['field_collection_item'][$sec_value]['field_media_video_section']['#items']) != 0) {
				...
				if ($video_count > 0 && $section_place == 'Right') {
				...
				for($j = 0; $j < $video_count; $j++)
				{
					...
					if (isset($content['field_media_sections'][$i]['entity']['field_collection_item'][$sec_value]['field_media_video_section'][$j]['entity']['field_collection_item'][$video_value]['field_media_video_desc']['#items']))
					...
					if ($section_place == 'Left' || $section_place == '') {
						$left_pane .= '<div class="relatedVideos clearfix ">
										<a href="'.$section_video_href.'" class="relatedVideosImage ">
											<img alt="'.$section_video_title.'" src="'.$media_file_path.$section_video_thumbnail.'">
			...
			<div class="initiatives">
				...
				<div class="initiativeMain layoutWide">
					<div id="colMain">
						...
						if (isset($node->field_media_video_info[LANGUAGE_NONE][0]['value']) || isset($node->field_media_video_info_title[LANGUAGE_NONE][0]['value']) ) {
							//$strip_field_media_video_info = strip_tags($node->field_media_video_info[LANGUAGE_NONE][0]['safe_value']);
							
							if ( isset($node->field_media_video_info_more[LANGUAGE_NONE]) ) {
								//$field_media_video_info_more = $node->field_media_video_info_more[LANGUAGE_NONE][0]['value'];
								$field_media_video_info = '<h2 class="infomoreh2">'. $node->field_media_video_info[LANGUAGE_NONE][0]['safe_value'] .' </h2><span style="display: none;" class="infomore"></span>';
							}
							else {
								$field_media_video_info = "<br/>".$node->field_media_video_info[LANGUAGE_NONE][0]['value']."<br/><br/>";
							}
						?>
						<div class="mod full caption clearfix">
							<h1><?php echo (isset($node->field_media_video_info_title[LANGUAGE_NONE])?$node->field_media_video_info_title[LANGUAGE_NONE][0]['value']:'');?></h1>
							<?php echo (isset($node->field_media_video_info[LANGUAGE_NONE])?$field_media_video_info:'');?>
						</div>
						<?php
						}
						?>
